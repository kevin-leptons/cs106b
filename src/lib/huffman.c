...
static int _huff_pqueue(struct huff_node nodes[HUFF_NCODE], struct pqueue *q)
{
    ...
    for (i = 0; i < HUFF_NCODE; i++) {
        ...
        btree_node = btree_mknode(NULL, NULL, nodes + i);
        if (btree_node == NULL)
            return -1;
...
static int _huff_tree(struct pqueue *q, struct btree *tree)
{
    ...
    for (; q->size > 1;) {
        ...
        if (clone == NULL)
            return -1;
        clone->data = malloc(sizeof(struct huff_node));
        if (clone->data == NULL)
            return -1;
...
static int _huff_wencode(struct bitvec *map, int sfd, int dfd)
{
    ...
    if (byfile_init(&ifile, sfd))
        return -1;
    if (bifile_init(&ofile, dfd, 0))
        return -1;

    byte_count = 0;
    bit_count = 0;
    for (;;) {
        r = byfile_read(&ifile, &code);
        if (r == 0)
            break;
        if (r < 0)
            return -1;
...
static int _huff_retree(int sfd, struct huff_info *info, struct btree *tree)
{
    ...
    // root node
    root = btree_mknode(NULL, NULL, NULL);
    if (root == NULL)
        goto ERROR;
    tree->root = root; 
    ...
    for (; queue.size > 0;) {
        ...
        // make left node
        if (byfile_read(&ifile, &next_byte) != 1)
            goto ERROR;
        rbyte_count += 1;
        if (next_byte) {
            cnode = btree_mknode(NULL, NULL, NULL);
            if (cnode == NULL)
                goto ERROR;
        ...
        if (node->left == NULL && node->right == NULL) {
            if (byfile_read(&ifile, &next_byte) != 1)
                goto ERROR;
            rbyte_count += 1;
            huff_node = malloc(sizeof(*huff_node));
            if (huff_node == NULL)
                goto ERROR;
