...
static int _ngram_init(struct ngram *ngram, size_t size, const char *file)
{
    ...
    for (;;) {
        ...
        if (svmap_get(&ngram->map, &window, (void *) &value)) {
            if (espace_catch(CS106B_EKEY)) {
...
static char * _ngram_make(struct ngram *ngram, size_t size)
{
    ...
    if (rrand(0, keys->size - 1, &findex))
        return NULL;
    if (vector_get(keys, findex, (void *) &key))
        return NULL;
    ...
    for (nword = 0; nword < size; nword++) {
        // random next word
        if (svmap_get(&ngram->map, &window, (void *) &next_words))
            return NULL;
        if (rrand(0, next_words->size - 1, &windex))
            return NULL;
        if (vector_get(next_words, windex, (void *) &next_word))
            return NULL;

        // pick first word in window, push to queue of result words
        // remove that word in window
        if (vector_get(&window, 0, (void *) &word))
            return NULL;
