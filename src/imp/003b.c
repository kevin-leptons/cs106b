...
struct grm
{
    struct htab map;      // map <symbol> => vector<vector<token>>
};
...
static int _grm_init(struct grm *grm, const char *file)
{
    ...
    if (cs106b_malloc((void *) &fdata, dsize))
        goto ERROR;
    for (p = fdata; ;) {
...
static struct vector * _grm_expand(struct grm *grm, struct grm_token *token)
{
    ...
    if (token->type != GRM_TOKEN_NTERM)
        goto ERROR;
    if (htab_get(&grm->map, token->value, (void *) &rside))
        goto ERROR;
    if (rside->size == 0)
        goto ERROR;
    ...
    if (rrand(0, rside->size - 1, &rcase))
        goto ERROR;
    if (vector_get(rside, rcase, (void *) &tcase))
        goto ERROR;
    if (tcase->size == 0)
        goto ERROR;
    ...
    for (i = 0; i < tcase->size; i++) {
        if (vector_get(tcase, i, (void *) &token))
            goto ERROR;
        if (token->type == GRM_TOKEN_NTERM) {
            ...
            for (k = 0; k < exp_tokens->size; k++) {
                if (vector_get(exp_tokens, k, (void *) &exp_token))
                    goto ERROR;
...
static int _grm_add_expr(struct grm *grm, const char *expr)
{
    ...
    if (vector_get(tokens, 0, (void *) &key_token))
        return -1;
    if (key_token->type != GRM_TOKEN_NTERM)
        return -1;
    if (vector_get(tokens, 1, (void *) &token))
        return -1;
    ...
    for (i = 2; i < tokens->size; i++) {
        if (rside_part == NULL) {
            ...
            if (rside_part == NULL)
                goto ERROR;
            vector_init(rside_part);
        }

        if (vector_get(tokens, i, (void *) &token))
            return -1;
        if (token->type == GRM_TOKEN_ASGN) {
            goto ERROR;
    ...
    // put to grammar map
    if (htab_set(&grm->map, key_token->value, rside)) 
        goto ERROR;
